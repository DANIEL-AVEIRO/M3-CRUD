...
import { productList } from "../utils/apiService";
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center">
        <Link
          to="/product/create/"
          className="px-6 py-4 inline-block text-white rounded-lg cursor-pointer bg-green-600"
        >
          Add New Product
        </Link>
        <input
        ...
        <tbody className="p-4 border border-solid border-black">
          {filteredProducts.length > 0 ? (
                ...
                >
                  ...
                  <td className="p-4 border border-solid border-black text-center">
                    <Link
                      ...
                      className="px-6 py-4 inline-block text-white rounded-lg cursor-pointer bg-yellow-500"
                    >
                      Edit
                    </Link>
                    <button
                      onClick={() => handleDelete(product.id)}
                      className="px-6 py-4 inline-block text-white ms-4 rounded-lg cursor-pointer bg-red-500"
                    >
                      Delete
                    </button>
                  </td>
            ...
            <tr className="p-4 border border-solid border-black">
              <td
                colSpan={4}
                className="p-4 border border-solid border-black text-center"
              >
                No products found
              </td>
            </tr>
